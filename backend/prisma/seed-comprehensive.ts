...
async function main() {
  ...
  for (const menteeData of menteesData) {
    const { email, ...data } = menteeData;
    const mentee = await prisma.mentee.create({ data });
    mentees.push(mentee);
...
Machine Learning is a powerful tool, but success requires understanding both the technical and practical aspects. Start with simple problems and gradually tackle more complex challenges.`,
  ...
  await prisma.articleTag.createMany({
    data: [
      { articleId: articles[0].id, tagId: tags.find(t => t.name === 'react')?.id! },
      { articleId: articles[0].id, tagId: tags.find(t => t.name === 'javascript')?.id! },
      { articleId: articles[1].id, tagId: tags.find(t => t.name === 'nodejs')?.id! },
      { articleId: articles[1].id, tagId: tags.find(t => t.name === 'webdev')?.id! },
      { articleId: articles[2].id, tagId: tags.find(t => t.name === 'webdev')?.id! },
      { articleId: articles[3].id, tagId: tags.find(t => t.name === 'devops')?.id! },
      { articleId: articles[4].id, tagId: tags.find(t => t.name === 'python')?.id! }
    ]
  });
...
For context, I want to develop mobile apps efficiently while maintaining good performance and user experience.`
  ...
  await prisma.questionTag.createMany({
    data: [
      { questionId: questions[0].id, tagId: tags.find(t => t.name === 'react')?.id! },
      { questionId: questions[0].id, tagId: tags.find(t => t.name === 'javascript')?.id! },
      { questionId: questions[1].id, tagId: tags.find(t => t.name === 'nodejs')?.id! },
      { questionId: questions[2].id, tagId: tags.find(t => t.name === 'javascript')?.id! },
      { questionId: questions[2].id, tagId: tags.find(t => t.name === 'career')?.id! },
      { questionId: questions[3].id, tagId: tags.find(t => t.name === 'python')?.id! },
      { questionId: questions[3].id, tagId: tags.find(t => t.name === 'career')?.id! },
      { questionId: questions[4].id, tagId: tags.find(t => t.name === 'career')?.id! }
    ]
  });
